...
{
   public partial class ListarPoliticas : FastConsigPageBase
   {
      ...
      protected void PoliticasRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
      {
         if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
         {
            var o = (Politicas)e.Item.DataItem;
      ...
      private void CarregarPoliticas()
      {
         try
         {
            PoliticasRepeater.DataSource = PropostaService.GetInstance().ListarPoliticas().OrderBy(c => c.Descricao).ToList();
            PoliticasRepeater.DataBind();
         }
         catch (Exception ex)
         {
            LogService.GetInstance().GravarLogDebug(ex, ex.Message);
            ShowMessage(TipoMensagem.Erro, string.Format(MensagemTexto.MsgPadraoErro, "Consulta de Políticas"));
         }
      }
