<%@ Page Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="EditarBanco.aspx.cs" Inherits="FastConsig.Core.Web.Configuracoes.EditarBanco" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
   <style type="text/css">
      .panel-nomargim {
         margin-right: 0;
         margin-left: 0;
         padding-left: 0;
         padding-right: 0;
      }

      .panel-invisible {
         background-color: transparent;
         -moz-box-shadow: none;
         -webkit-box-shadow: none;
         box-shadow: none;
      }

      .feed-title > span {
         font-weight: 300;
         font-size: 18px;
      }

      .dataTables_paginate {
         float: left !important;
      }
   </style>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
   <div class="row">
      <div class="col-lg-12">
         <div class="card">
            <div class="card-body">
               <div class="row">
                  <div class="col-lg-5">
                     <div class="mt-4">
                        <div class="d-flex flex-wrap gap-3 mt-3 mx-auto">
                           <fmk:CustomLinkButton ID="btnSalvar" runat="server" class="btn btn-primary" OnClick="btnSalvar_Click" DialogConfirmarAcao="true">Salvar</fmk:CustomLinkButton>
                           <fmk:CustomLinkButton ID="btnExcluir" runat="server" class="btn btn-outline-danger" OnClick="btnExcluir_Click" DialogConfirmarExclusao="true">Excluir</fmk:CustomLinkButton>
                           <fmk:CustomLinkButton ID="btnVoltar" runat="server" class="btn btn-secondary waves-effect" OnClick="btnVoltar_Click" CausesValidation="false">Voltar</fmk:CustomLinkButton>
                        </div>

                        <div class="mb-3">
                        </div>

                        <div class="row">
                           <div class="col-md-3">
                              <div class="mb-1">
                                 <label class="form-label" for="txtCodigo">Código</label>
                                 <fmk:CustomTextBox ID="txtCodigo" runat="server"
                                    CssClass="form-control"
                                    TabIndex="1"
                                    placeholder="Banco"
                                    BindSelectedType="Message"
                                    Required="true"
                                    MaxLength="3"
                                    Enabled="true"
                                    FormatType="Integer"
                                    onkeypress="return isNumber(event)"
                                    onpaste="return false;"
                                    autocomplete="off"
                                    BindSourceType="Message"
                                    BindSourceText="Bancos.Banco"
                                    BindSourceValue="Bancos.Banco"></fmk:CustomTextBox>
                              </div>
                           </div>
                        </div>

                        <div class="row">
                           <div class="col-md-3">
                              <div class="mb-1">
                                 <label class="form-label" for="txtCBCDataPrev">CBC DataPrev</label>
                                 <fmk:CustomTextBox ID="txtCBCDataPrev" runat="server"
                                    CssClass="form-control"
                                    TabIndex="1"
                                    placeholder="CBC DataPrev"
                                    BindSelectedType="Message"
                                    Required="false"
                                    MaxLength="3"
                                    Enabled="true"
                                    FormatType="Integer"
                                    onkeypress="return isNumber(event)"
                                    onpaste="return false;"
                                    autocomplete="off"
                                    BindSourceType="Message"
                                    BindSourceText="Bancos.CBCDataPrev"
                                    BindSourceValue="Bancos.CBCDataPrev"></fmk:CustomTextBox>
                              </div>
                           </div>
                        </div>

                        <div class="mb-3">
                           <label class="form-label" for="txtDescricao">Descrição</label>
                           <fmk:CustomTextBox ID="txtDescricao" runat="server"
                              CssClass="form-control"
                              TabIndex="2"
                              placeholder="Nome"
                              BindSelectedType="Message"
                              Required="True"
                              Enabled="true"
                              MaxLength="150"
                              FormatType="Text"
                              onpaste="return false;"
                              autocomplete="off"
                              BindSourceType="Message"
                              BindSourceText="Bancos.Nome"
                              BindSourceValue="Bancos.Nome"></fmk:CustomTextBox>
                        </div>

                        <div class="mb-3">
                           <label class="form-label" for="txtCNPJ">CNPJ</label>
                           <fmk:CustomTextBox ID="txtCNPJ" runat="server"
                              CssClass="form-control"
                              TabIndex="2"
                              placeholder="Cnpj"
                              BindSelectedType="Message"
                              Required="false"
                              Enabled="true"
                              MaxLength="14"
                              FormatType="Text"
                              onkeypress="return isNumber(event)"
                              onpaste="return false;"
                              autocomplete="off"
                              BindSourceType="Message"
                              BindSourceText="Bancos.Cnpj"
                              BindSourceValue="Bancos.Cnpj"></fmk:CustomTextBox>
                        </div>

                        <div class="mb-3">
                           <label class="form-label" for="txtISPB">ISPB</label>
                           <fmk:CustomTextBox ID="txtISPB" runat="server"
                              CssClass="form-control"
                              TabIndex="2"
                              placeholder="ISPB"
                              BindSelectedType="Message"
                              Required="false"
                              Enabled="true"
                              MaxLength="8"
                              FormatType="Text"
                              onkeypress="return isNumber(event)"
                              onpaste="return false;"
                              autocomplete="off"
                              BindSourceType="Message"
                              BindSourceText="Bancos.ISPB"
                              BindSourceValue="Bancos.ISPB"></fmk:CustomTextBox>
                        </div>

                        <div class="mb-3">
                           <div class="form-check">
                              <fmk:CustomCheckBox
                                 ID="chkSelecionavel"
                                 runat="server"
                                 CssClass="form-check-input"
                                 BindSourceText="Bancos.Ativo"
                                 BindSourceValue="Bancos.Ativo"
                                 TabIndex="3"
                                 BindSourceType="Message"
                                 Text="" />
                              <label class="form-check-label" for="chkSelecionavel">Ativo?</label>
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="ScriptContent" runat="server">
</asp:Content>
