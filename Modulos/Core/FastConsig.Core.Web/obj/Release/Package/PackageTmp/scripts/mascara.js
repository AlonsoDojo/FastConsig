...
function verificaTelefone(campo)
{
    ...
    var filtraCampoNumero = filtraNumeros(valorCampo);
    var tamanhoCampoNumero = (filtraCampoNumero.toString()).length;
    campo.maxLength = 15;

    if (tamanhoCampoNumero <= 10)
        formataOitoDigitos(campo, null);
    else if (tamanhoCampoNumero > 10 && tamanhoCampoNumero <= 11)
        formataNoveDigitos(campo, null);
}

function formataOitoDigitos(campo, evt) {
    //(88) 8888-8888 
    var xPos = PosicaoCursor(campo);
    evt = getEvent(evt);
    var tecla = getKeyCode(evt);
...
function formataNoveDigitos(campo, evt) {
    //(99) 99999-9999
    var xPos = PosicaoCursor(campo);
    evt = getEvent(evt);
    var tecla = getKeyCode(evt);
...
function verificaCpfCnpj(campo, evt) {
    ...
    if (tamanhoCampoNumero <= 11)
        formataCPF(campo, evt);
    else if (tamanhoCampoNumero > 11 && tamanhoCampoNumero <= 14)
