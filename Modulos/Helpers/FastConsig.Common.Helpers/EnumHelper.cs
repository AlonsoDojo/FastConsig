...
namespace FastConsig.Common.Helpers
{
   public static class EnumHelper
   {
      ...
      /// <summary>
      /// Converte uma String para o Valor de um Enum
      /// </summary>
      /// <typeparam name="T">Enum</typeparam>
      /// <param name="enumString">Valor a Ser Convertido</param>
      /// <returns></returns>
      public static T ToEnum<T>(this string enumString)
      {
         T retorno = (T)Enum.Parse(typeof(T), enumString);
         if (!Enum.IsDefined(typeof(T), retorno) && !enumString.ToString().Contains(","))
         {
            throw new InvalidOperationException(
                $"{enumString} is not an underlying value of the YourEnum enumeration."
            );
         }
         return retorno;
      }
